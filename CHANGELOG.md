...
- `BINARY`, `VARBINARY`, and `BLOB` columns now return Node `Buffer` on both backends (mapepire previously returned an uppercase hex string). Node `Buffer` is also accepted as a parameter for binary columns on both backends; the mapepire wrapper converts it to a lowercase hex binding transparently. The idb backend now defensively copies `Buffer` parameters before binding — idb-pconnector's native implementation uses the caller's `Buffer` as scratch space at sizes ≥ 64 KiB, so without the copy the caller's original `Buffer` would be corrupted after the call. See `docs/BACKEND-DIFFERENCES.md` for measured size ceilings (idb round-trips up to at least 128 MiB; mapepire tops out at ~24 MiB due to a 50 MiB server-side WebSocket message cap and the hex-encoding 2× overhead).
